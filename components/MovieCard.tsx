...
import { FaChevronCircleDown, FaHeart, FaPlay } from "react-icons/fa"
...
const Moviecard = ({movie}:any) => {
  ...
  return (
    ...
    <Image width={200}   height={200} src={movie.thumbnailUrl} alt="Movie" draggable={false} className="
      cursor-pointer
      object-cover
      transition
      duration
      ...
      <div className="
        ...
        <div className="flex flex-row items-center gap-3">
          <div  className="cursor-pointer w-6 h-6 lg:w-10 lg:h-10 bg-white rounded-full flex justify-center items-center transition hover:bg-neutral-300">
            <FaPlay className="text-black w-4 lg:w-6" onClick={()=>router.push(`/${movie.id}`)} />
          </div>
          <FaHeart  />
          <div  className="cursor-pointer ml-auto group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center transition hover:border-neutral-300">
            <FaChevronCircleDown className="text-white group-hover/item:text-neutral-300 w-4 lg:w-6" />
          </div>
        </div>
